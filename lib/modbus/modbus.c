...
/* Reads the content of remote device and put the data into an array */
int modbus_read_words(modbus_t *ctx, int function, uint16_t *dest)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
    ...
    int req_length = ctx->backend->build_request_basis(ctx, function, 0, 0, req);
...
/* Reads the content of remote device and put the data into an array */
int _modbus_receive_bytes(modbus_t *ctx, uint8_t *msg, int length_to_read)
{
    int rc;
    fd_set rset;
    struct timeval tv;
...
int modbus_read_bytes(modbus_t *ctx, int addr, int nb, uint8_t *dest)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
    ...
    int req_length = ctx->backend->build_request_basis(ctx, 0, addr, 0, req);

    int rc = send_msg(ctx, req, req_length);
    if (rc > 0) {
        rc = _modbus_receive_bytes(ctx, rsp, nb);
        if (rc == -1)
            return -1;
